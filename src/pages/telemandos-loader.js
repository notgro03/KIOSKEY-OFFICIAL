...
const renderUnavailable = (container) => {
  container.innerHTML = `
    <div style="text-align: center; padding: 60px 20px; color: rgba(255,255,255,0.7); grid-column: 1/-1;">
      ...
      <p>No pudimos conectar con nuestro catálogo de telemandos. Intenta nuevamente más tarde.</p>
    </div>
  `;
};
...
export async function loadTelemandos() {
  ...
  container.innerHTML = '<div style="text-align: center; padding: 40px; color: white;"><i class="fas fa-spinner fa-spin" style="font-size: 32px;"></i><p>Cargando telemandos...</p></div>';
  ...
  try {
    ...
    if (!products || products.length === 0) {
      container.innerHTML = `
        <div style="text-align: center; padding: 60px 20px; color: rgba(255,255,255,0.7); grid-column: 1/-1;">
          <i class="fas fa-car" style="font-size: 64px; margin-bottom: 20px; opacity: 0.5;"></i>
          <h3 style="font-size: 24px; margin-bottom: 12px;">No hay telemandos disponibles</h3>
    ...
    container.innerHTML = products.map(product => {
      const title = product.title || `${product.brand ?? ''} ${product.model ?? ''}`.trim() || 'Telemando disponible';
        ...
        <div class="product-card" style="background: linear-gradient(135deg, rgba(15, 15, 15, 0.9), rgba(25, 25, 25, 0.8)); border-radius: 20px; overflow: hidden; border: 2px solid rgba(255,255,255,0.1); transition: all 0.4s ease;">
          <div class="product-image" style="height: 250px; overflow: hidden; position: relative; background: rgba(0,0,0,0.3);">
            ${product.image_url
              ...
              : `<div style="width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: rgba(255,255,255,0.05);"><i class="fas fa-car" style="font-size: 64px; color: rgba(255,255,255,0.2);"></i></div>`
            }
            ${stock > 0
    ...
    document.querySelectorAll('.btn-add-cart').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (product) {
          ...
          const message = encodeURIComponent(`Hola, me interesa el telemando ${title}. ¿Podrían brindarme información sobre precio y disponibilidad?`);
