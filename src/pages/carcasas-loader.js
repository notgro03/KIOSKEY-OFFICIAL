...
const renderUnavailable = (container) => {
  container.innerHTML = `
    <div style="text-align: center; padding: 60px 20px; color: rgba(255,255,255,0.7); grid-column: 1/-1;">
      ...
      <p>No pudimos conectar con nuestro catálogo de carcasas. Intenta nuevamente más tarde.</p>
    </div>
  `;
};
...
export async function loadCarcasas() {
  ...
  try {
    const { data: products, error } = await supabase
      .from('carcasas')
      .select('id, brand, model, description, image_url, price, stock, color, active')
      .eq('active', true)
    ...
    container.innerHTML = products.map(product => {
      const title = product.title || `${product.brand ?? ''} ${product.model ?? ''}`.trim() || 'Carcasa disponible';
      ...
      return `
        <div class="product-card" style="background: linear-gradient(135deg, rgba(15, 15, 15, 0.9), rgba(25, 25, 25, 0.8)); border-radius: 20px; overflow: hidden; border: 2px solid rgba(255,255,255,0.1); transition: all 0.4s ease;">
          ...
          <div class="product-info" style="padding: 24px;">
            ...
            ` : ''}
            ${color ? `
              <p style="color: rgba(255,255,255,0.6); font-size: 14px; margin-bottom: 12px;"><i class="fas fa-palette"></i> Color: ${color}</p>
            ` : ''}
            ${description ? `
    ...
    document.querySelectorAll('.btn-add-cart').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (product) {
          ...
          const message = encodeURIComponent(`Hola, me interesa la carcasa ${title}. ¿Podrían brindarme información sobre precio y disponibilidad?`);
