...
const renderUnavailable = (container) => {
  container.innerHTML = `
    <div style="text-align: center; padding: 60px 20px; color: rgba(255,255,255,0.7); grid-column: 1/-1;">
      ...
      <p>No pudimos conectar con nuestro catálogo de accesorios. Intenta nuevamente más tarde.</p>
    </div>
  `;
};
...
export async function loadAccesorios() {
  ...
  try {
    ...
    if (categoryError) {
      console.error('Error loading accessories category:', categoryError);
      renderUnavailable(container);
      return;
    }
    ...
    container.innerHTML = products.map(product => {
      const title = product.title || product.name || 'Accesorio disponible';
    ...
    document.querySelectorAll('.btn-add-cart').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (product) {
          const title = product.title || product.name || 'Accesorio';
          const message = encodeURIComponent(`Hola, me interesa el accesorio ${title}. ¿Podrían brindarme información sobre precio y disponibilidad?`);
