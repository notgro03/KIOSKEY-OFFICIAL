...
async function loadProducts() {
  ...
  container.innerHTML = products.map(product => `
    <div class="item-card">
      <div class="item-info">
        <h4><i class="fas ${product.icon}"></i> ${product.title}</h4>
        <p>${product.description.substring(0, 100)}...</p>
        <p style="font-size: 12px; color: rgba(255,255,255,0.5);">Categoría: ${product.category} | Orden: ${product.order_position}</p>
      </div>
      <div class="item-actions">
        <button class="btn-icon" onclick="editProduct('${product.id}')" title="Editar">
          <i class="fas fa-edit"></i>
        </button>
        <button class="btn-icon" onclick="toggleProductActive('${product.id}', ${!product.active})" title="${product.active ? 'Desactivar' : 'Activar'}">
          <i class="fas fa-${product.active ? 'eye' : 'eye-slash'}"></i>
        </button>
...
async function handleProductSubmit(e) {
  ...
  const link_url = document.getElementById('productLinkUrl').value;
  const link_text = document.getElementById('productLinkText').value;
  const order_position = parseInt(document.getElementById('productOrder').value);
...
window.editProduct = async function(id) {
  ...
  document.getElementById('productLinkUrl').value = product.link_url;
  document.getElementById('productLinkText').value = product.link_text;
  document.getElementById('productOrder').value = product.order_position;
  ...
  features.forEach(feature => {
    ...
    div.innerHTML = `
      ...
      <button type="button" class="btn-icon" onclick="removeFeature(this)">
...
// Features management for products
window.addFeature = function() {
  const featuresList = document.getElementById('featuresList');
  const div = document.createElement('div');
  div.className = 'feature-item';
  div.innerHTML = `
    <input type="text" class="feature-input" placeholder="Nueva característica">
    <button type="button" class="btn-icon" onclick="removeFeature(this)">
      <i class="fas fa-times"></i>
    </button>
  `;
...
function resetFeatures() {
  ...
  featuresList.innerHTML = `
    <div class="feature-item">
      <input type="text" class="feature-input" placeholder="Característica 1">
      <button type="button" class="btn-icon" onclick="removeFeature(this)">
        <i class="fas fa-times"></i>
      </button>
    </div>
